...
describe('renameSeries - Integration Test', () => {
  let testSeriesId;
  ...
  test('should trigger RenameSeries command and return a queued command', async () => {
    const result = await sonarrService.renameSeries(testSeriesId);

    // Sonarr returns a command resource with id, name, status
    expect(result).toHaveProperty('id');
    expect(result).toHaveProperty('name', 'RenameSeries');
  ...
  test('should be able to check command status via GET /command/{id}', async () => {
    // Trigger rename
    const cmd = await sonarrService.renameSeries(testSeriesId);
